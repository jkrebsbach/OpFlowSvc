...
namespace OpFlow.Service.Controllers
{
    ...
    public class TrayProposalController : ApiController
    {
        ...
        public async Task<HttpResponseMessage> GetTrayRationalization()
        {
            ...
            var sqlHelper = new ReadOnlySqlHelper();
        ...
        public async Task<HttpResponseMessage> GetHomeDashboard()
        {
            ...
            var proposedTrays = await sqlHelper.GetProposedTrayDashboard(user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> GetProposedTrays()
        {
            ...
            var trayId = await sqlHelper.GetProposedTrays(null, user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> GetProposedTray(int trayProposalId)
        {
            ...
            var documents = await sqlHelper.GetProposedTrayApprovalDocuments(trayProposalId, user.SelectedLocation);
            ...
            var cardCategories = await sqlHelper.GetProposedTrayCardCategories(trayProposalId, user.SelectedLocation);
            var history = await sqlHelper.GetProposedTrayLog(trayProposalId, user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> GetStatusLog(int trayProposalId)
        {
            ...
            var statusLog = await sqlHelper.GetProposedTrayStatusLog(trayProposalId, user.SelectedLocation);
        ...
        [HttpGet]
        public async Task<HttpResponseMessage> GetCardOverlap(int trayProposalId, string sortBy = null, int? overlapPcnt = 0)
        {
            var user = await CacheUtil.GetUserSecurity();
            var sqlHelper = new SqlHelper();
        ...
        public async Task<HttpResponseMessage> UpdateProposalNotes(int trayProposalId, [FromBody] ProposedTrayNotesPost request)
        {
            ...
            var cases = await sqlHelper.UpdateProposedTrayComments(trayProposalId, request.Notes, user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> PutTrayApproval(int trayProposalId, int typeId)
        {
            ...
            var readOnlySqlHelper = new ReadOnlySqlHelper();
            var sqlHelper = new SqlHelper();
            // validate location auth
            var proposedTray = (await readOnlySqlHelper.GetProposedTrays(trayProposalId, user.SelectedLocation)).First();
            ...
            try
            {
                ...
                await sqlHelper.UpdateProposedTrayApproval(trayProposalId, fileName, typeId, user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> PutOrgChart(string type, int? tray)
        {
            ...
            try
            {
                ...
                var orgChartId = await sqlHelper.UpdateOrgChartAttachment(type, tray, fileName, user.SelectedLocation);
                ...
                var proposals = await readOnlySqlHelper.GetProposedTrays(null, user.SelectedLocation); 
                var orgCharts = await readOnlySqlHelper.GetOrgChartAttachments(user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> DeleteOrgChart(string type, int? id)
        {
            ...
            try
            {
                ...
                if (type == "T")
                {
                    var proposalChecks = await readOnlysqlHelper.GetProposedTrays(id, user.SelectedLocation);
                    var proposal = proposalChecks.First();

                    await sqlHelper.DeleteProposedTrayOrgChart(null, id, user.SelectedLocation);
                ...
                {
                    var attachments = await readOnlysqlHelper.GetOrgChartAttachments(user.SelectedLocation);
                    var attachment = attachments.First(a => a.OrgChartID == id);

                    await sqlHelper.DeleteProposedTrayOrgChart(id, null, user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> PutImplementation(int target)
        {
            ...
            try
            {
                ...
                var attachmentId = await sqlHelper.UpdateImplementationAttachment(target, fileName, user.SelectedLocation);
                ...
                var attachments = await readOnlysqlHelper.GetImplementationAttachments(user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> DeleteImplementationAttachment(int attachmentId)
        {
            ...
            attachments = await readOnlysqlHelper.GetImplementationAttachments(user.SelectedLocation);
            var result = TrayImplementation.GetImplementationSteps(attachments);
        ...
        public async Task<HttpResponseMessage> GetAuditSummary(string auditType, int? trayId, int? specialtyId, DateTime startDate, DateTime endDate)
        {
            ...
            var audits = await sqlHelper.GetProposedTrayAuditSummary(startDate, endDate, user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> GetSurgeryTraySchedule(int surgeryId)
        {
            ...
            var trayProposals = await sqlHelper.GetSurgeryTraySchedule(surgeryId, null, user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> GetTraySchedule(int? surgeonId, int? trayProposalId, DateTime startDate, DateTime endDate)
        {
            ...
            var surgeries = await sqlHelper.SearchCaseTraySchedule(surgeonId, trayProposalId, startDate, endDate, user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> GetCaseProfile(int? caseProfileId)
        {
            ...
            if (caseProfileId.HasValue)
                caseProfile = await sqlHelper.GetCaseProfile(caseProfileId.Value, user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> GetCaseProfiles(int? surgeryId = null)
        {
            ...
            if (surgeryId.HasValue)
                caseProfiles = new List<CaseProfile>() { await sqlHelper.GetSurgeryCaseProfile(surgeryId.Value, user.SelectedLocation) };
            else
                caseProfiles = await readOnlysqlHelper.GetCaseProfiles(user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> UpdateConsolidationPlan([FromBody] ConsolidationPlanPost post)
        {
            ...
            await sqlHelper.UpdateConsolidationPlan(post.Proposals, user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> UpdateSurgeryCaseProfile(int surgeryId, int caseProfileId, [FromBody] CaseProfileSchedulePost post)
        {
            ...
            await sqlHelper.UpdateSurgeryCaseProfile(surgeryId, caseProfileId, post.Questions, user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> GetTrayPhoto(int trayProposalId)
        {
            ...
            var trayProposal = await readOnlysqlHelper.GetProposedTrays(trayProposalId, user.SelectedLocation);
            if (trayProposal.Any())
            {
        ...
        public async Task<IHttpActionResult> RotateTrayImage(int trayProposalId, int direction)
        {
            ...
            var trayProposals = await readOnlysqlHelper.GetProposedTrays(trayProposalId, user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> GetTrayProposalLog(int proposedTrayId, [FromBody] TrayProposalLogPost post)
        {
            ...
            var logs = await readOnlysqlHelper.GetProposedTrayLog(proposedTrayId, user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> GetTrayProposalLogCsv(int trayProposalId, int logId, [FromBody] TrayProposalLogPost post)
        {
            ...
            var proposal = await readOnlysqlHelper.GetProposedTrays(trayProposalId, user.SelectedLocation);
            var proposals = await readOnlysqlHelper.GetProposedTrayLog(trayProposalId, user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> UpdateSurgeonPreference(int? preferenceId, [FromBody] SurgeonPreferencePost post)
        {
            ...
            var surgeonPreferences = await readOnlysqlHelper.GetSurgeonPreferences(user.SelectedLocation);
            var trayGroups = await readOnlysqlHelper.GetTrayGroups(user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> UpdateCommunicationRoles(int trayProposalId, [FromBody] TrayCommunicationRolePost post)
        {
            ...
            var users = await readOnlysqlHelper.SearchUsers(null, null, null, user.SelectedLocation);
            var proposals = await readOnlysqlHelper.GetProposedTrays(null, user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> AddComparableInstrument(int trayProposalId, int instrumentId, int comparableInstrumentId)
        {
            ...
            var instruments = await readOnlysqlHelper.GetProposedTrayInstruments(trayProposalId, user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> PutTrayCsv([FromBody] TrayProposalCsvExportPost post)
        {
            ...
            {
                var proposedTray = await readOnlysqlHelper.GetProposedTrays(trayProposalId, user.SelectedLocation);
        ...
        private async Task<HttpResponseMessage> TraySummaryCsv(List<int> trayProposalIds)
        {
            ...
            foreach (var trayProposalId in trayProposalIds)
            {
                ...
                var proposedTray = (await readOnlysqlHelper.GetProposedTrays(trayProposalId, user.SelectedLocation)).First();
                var proposedInstruments = await readOnlysqlHelper.GetProposedTrayInstruments(trayProposalId, user.SelectedLocation);
                var sourceTrays = await readOnlysqlHelper.GetSourceTraySummary(trayProposalId, user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> GetTrayScheduling()
        {
            ...
            var result = await readOnlysqlHelper.GetProposedTraySchedule(null, null, user.UserID, user.SelectedLocation);
            var rules = await readOnlysqlHelper.GetProposedTrayScheduleRules(user.UserID, user.SelectedLocation);
        ...
        public async Task<HttpResponseMessage> GetTrayAuditCsv(int? trayProposalId, string filter, DateTime? startDate, DateTime? endDate)
        {
            ...
            {
                audits.AddRange(await readOnlysqlHelper.GetProposedTrayAudits(trayProposalId, startDate, endDate, user.SelectedLocation));
            }
            if (filter == null || filter == "C")
            {
                audits.AddRange(await readOnlysqlHelper.GetProposedTrayCounts(trayProposalId, startDate, endDate, user.SelectedLocation));
            }
        ...
        public async Task<HttpResponseMessage> GetTrayDetailCsv([FromBody] TrayRationalizationDetailPost post)
        {
            ...
            var instruments = await readOnlysqlHelper.GetProposedTrayInstruments(post.TrayProposalID, user.SelectedLocation);
            ...
            {
                var rationalization = await readOnlysqlHelper.GetTrayRationalizationDetail(
                    post.TrayProposalID,
        ...
        public async Task<HttpResponseMessage> PostTrayRationalizationDetail([FromBody] TrayRationalizationDetailPost post)
        {
            ...
            var instruments = post.Rollup == "I" ?
                await readOnlysqlHelper.GetProposedTrayInstruments(post.TrayProposalID, user.SelectedLocation)
            ...
            foreach (var source in sources)
            {
                var trayDetail = post.Rollup == "I" ?
                    ...
                    : await sqlHelper.GetTrayItemCategories(source.Key.TrayItemID, user.SelectedLocation);
        ...
        { 
            ...
            var proposed = await readOnlysqlHelper.GetProposedTrayInstruments(trayProposalId, user.SelectedLocation);
            ...
            foreach (var tray in traySources)
            {
                ...
                if (tray.TrayType == "tray")
                {
                    ...
                    trayInstruments = await sqlHelper.GetTrayItemOverlaps(tray.TrayID, user.SelectedLocation);
                }
                else 
                {
                    trayInstruments = await sqlHelper.GetProposedTrayItemOverlaps(tray.TrayID, tray.TrayLogID, user.SelectedLocation);
                }
